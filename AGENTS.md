...
Policy and HR Copilot (H2): Deterministic HR entitlement engine with cited policy answers for BFSI workforce copilots.
Rendered from `hex-service-template`, so it starts at reference parity rather than converging
toward it. Package `hr_policy_answers`, environment prefix
`POLICYHR`, region `asia-southeast1`.
...
- `src/hr_policy_answers/domain/` is PURE stdlib. No web framework, no cloud SDK,
...
- **The exposure guard is derived from the IDENTITY BINDING, and from nothing else.** An end-user
  ...
  `POLICYHR_S2S_TOKEN` may never enter that decision: it authenticates a
...
- **The one adapter that declares `VERIFIED` must EARN it, and it is the one adapter that may not
  ...
  configured `POLICYHR_IAP_AUDIENCE`, three-state: unset or emptied REFUSES,
...
- **One profile read, three states, refused at import.** `config.PROFILE_CHOICE` resolves once at
  ...
  `POLICYHR_PROFILE` (mentioning it in a refusal message is wanted, reading
