"""FastAPI application for Policy and HR Copilot (H2).
...
``POLICYHR_PROFILE`` is NO CHOICE rather than a silent ``local``. Every
...
``POLICYHR_S2S_TOKEN`` is wrong in the most dangerous direction:
...
#: The action label the entitlement path records on an escalated review at the console.
...
_TOKEN_ENV = "POLICYHR_S2S_TOKEN"
_INSECURE_DEMO_ENV = "POLICYHR_ALLOW_INSECURE_DEMO"
_CORS_ORIGINS_ENV = "POLICYHR_CORS_ORIGINS"
_ALLOWED_CALLERS_ENV = "POLICYHR_S2S_ALLOWED_CALLERS"
_AUDIENCE_ENV = "POLICYHR_S2S_AUDIENCE"
_API_HOST_ENV = "POLICYHR_API_HOST"
...
def require_service_caller(request: Request) -> None:
    ...
    stays OPEN when ``POLICYHR_S2S_TOKEN`` is unset (loopback dev with zero
    secrets), so an UNSET ``POLICYHR_PROFILE`` must never be allowed to
    ...
    ``POLICYHR_ALLOW_INSECURE_DEMO=1``.
    """
    if not _request_choice(request).service_auth_configured:
        raise HTTPException(
            ...
            detail=(
                ...
                "POLICYHR_PROFILE is not set, so no authentication scheme "
                "has been chosen"
            ),
        )
...
_SERVICE_NAME = "hr-policy-answers"
...
_APP_TITLE = "Policy and HR Copilot"
...
#
# Note what is NOT in this expression: POLICYHR_S2S_TOKEN. A service
...
def triage(
    ...
    # The hand-off never fails an already-computed, already-audited result; the response says
...
@app.post("/v1/entitlement", response_model=EntitlementResponse, tags=["artifacts"])
def entitlement(
    request: EntitlementRequestModel,
    principal: Annotated[Principal, Depends(get_principal)],
) -> EntitlementResponse:
    """Compute an HR entitlement worksheet deterministically, and route consequential ones (R8).
    ...
    result = service.assess(
        EntitlementRequest(
            facts=EmployeeFacts(
                ...
                months_of_service=request.months_of_service,
                leave_taken_days=request.leave_taken_days,
                termination_linked=request.termination_linked,
            ),
    ...
    review_ref = routing.route(
        result, maker=principal.actor, tenant=principal.tenant, action=_ENTITLEMENT_ACTION
    )
