...
class AgentCard:
    ...
    provider: str = "hr-policy-answers"
    skills: tuple[AgentSkill, ...] = field(default_factory=tuple)


SKILLS: tuple[AgentSkill, ...] = (
    AgentSkill(
        id="assess_entitlement",
        name="HR entitlement assessment",
        description=(
            "Compute an HR leave entitlement deterministically from versioned, cited rule packs "
            "(balance, eligibility, approval path), then ROUTE consequential results to human "
            "review (rule R8). Every number is computed by pure code; the model never produces a "
            "day count or a verdict, and an unpacked jurisdiction fails closed to a human."
        ),
    ),
    AgentSkill(
        id="triage_case",
...
def _resolve_url(settings: Settings) -> str:
    ...
    return f"https://hr-policy-answers.{settings.region}.internal.example/a2a"
