...
DEFAULT_ACTOR = "hr-policy-answers-agent"

#: The action label an escalated entitlement carries to the console.
...
def assess_entitlement(
    ...
    """Compute an HR leave entitlement deterministically and route consequential results (R8).

    Every number returned is computed by the pure engine from versioned, cited rule packs, never
    by a model. A termination-linked question, a contested (overdrawn) balance, or a fact the packs
    cannot answer sets ``requires_human_review`` and is routed to the human-review console.

    Args:
      employee_ref: An opaque, fictional employee reference (never a real identifier).
      jurisdiction: The employing jurisdiction (SG, AU or JP in the shipped packs).
      employment_type: full_time or part_time.
      months_of_service: Completed months of continuous service.
      leave_taken_days: Leave days already taken this cycle.
      termination_linked: True when the question is tied to a termination or final pay.
      entitlement: The entitlement to compute (default annual_leave).
      actor: The verified identity this call is attributed to.
      tenant: Tenant partition asserted on an outbound review.
    ...
    request = EntitlementRequest(
        facts=EmployeeFacts(
            ...
            termination_linked=termination_linked,
        ),
...
TOOL_FUNCTIONS = (assess_entitlement, triage_case, verify_audit_trail)
