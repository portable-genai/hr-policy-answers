...
One env var (``POLICYHR_PROFILE``) selects the adapter family for every
...
class ProfileChoice:
    ...
    The variable is ``POLICYHR_PROFILE``, named once in
    :data:`_PROFILE_ENV`.
    ...
    ``os.environ.get("POLICYHR_PROFILE", "local")``: that fallback reads an
...
def _read_settings_file(path: Path | None = None) -> dict[str, Any]:
    ...
    An EXPLICIT path (argument or ``POLICYHR_SETTINGS``) that does not
...
REVIEW_ROUTING_ENV = "POLICYHR_REVIEW_ROUTING"
