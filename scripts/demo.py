...
below drives the actual entitlement engine, the actual hash-chained audit store and the actual
rule-R8 review router over the ``local`` profile, so a step that stops being true stops passing
rather than stops being mentioned. Every number the worksheet shows is computed by the pure
engine from versioned, cited rule packs; a model would narrate it, never produce it.
...
from hr_policy_answers.domain import (
    kernel,
    models,
)
...
SERVICE_NAME = "Policy and HR Copilot"
CATALOG_ID = "H2"
REPOSITORY = "hr-policy-answers"
...
#: A routine question: a Singapore full-timer well inside their annual-leave entitlement. The
#: engine computes a positive balance, so it is auto-approved and nothing is routed.
ROUTINE_CASE = models.EntitlementRequest(
    facts=models.EmployeeFacts(
...
#: A consequential question: a termination-linked final-pay entitlement in Australia. Consequential
#: by construction, so the engine escalates it whatever the arithmetic says (rule R8).
ESCALATING_CASE = models.EntitlementRequest(
    facts=models.EmployeeFacts(
...
#: The same consequential path with a synthetic national id planted in the employee reference, so
#: the redaction panel can prove the identifier is masked before the immutable write.
PII_CASE = models.EntitlementRequest(
    facts=models.EmployeeFacts(
        employee_ref="EMP NRIC " + PLANTED_NRIC + " (FICTIONAL)",
        jurisdiction="SG",
        employment_type="full_time",
...
STEPS: tuple[Step, ...] = (
    ...
    Step(
        key="routine",
        label="A routine entitlement: computed, cited, auto-approved",
        narration=(
            "A Singapore full-timer well inside their annual-leave entitlement. The balance comes "
            "from deterministic code and a cited rule pack, never from a model. Nothing is routed, "
            "because manufacturing a review for an ordinary balance trains reviewers to "
            "rubber-stamp."
        ),
    ),
    Step(
        key="escalation",
        label="A consequential entitlement: escalated AND routed (rule R8)",
        narration=(
            "A termination-linked final-pay entitlement. It is consequential by construction, so "
            "the engine escalates it whatever the arithmetic says. Setting the flag is not the "
            "escalation; routing is, in the same call that produced the worksheet."
        ),
    ),
    Step(
        key="redaction",
        ...
        narration=(
            "The same path, with a national id planted in the employee reference. The identifier "
            "is masked before anything is written, so the immutable record never contains it. "
...
class DemoRun:
    ...
    def _step_routine(self) -> Produced:
        return self._entitlement_panels(ROUTINE_CASE, expect_routing=False)

    def _step_escalation(self) -> Produced:
        return self._entitlement_panels(ESCALATING_CASE, expect_routing=True)

    def _step_redaction(self) -> Produced:
        panels, facts = self._entitlement_panels(PII_CASE, expect_routing=True)
        ...
        panels.append(
            Panel(
                ...
                rows=(
                    Row("Identifier in the employee reference", PLANTED_NRIC, "warn"),
                    Row(
    ...
    def _entitlement_panels(
        self, request: models.EntitlementRequest, *, expect_routing: bool
    ) -> Produced:
        result = self.service.assess(request, actor=ACTOR)
        self.cases += 1
        review_ref = ""
        action = "hr_policy_answers:entitlement"
        if result.requires_human_review:
            self.escalated += 1
            review_ref = self.container.review_router.route(
                result, maker=ACTOR, tenant=TENANT, action=action
            )
            self.routed += 1
        consistent = bool(review_ref) == expect_routing == result.requires_human_review

        def _num(value: float | None) -> str:
            return "n/a (fail-closed)" if value is None else f"{value:g}"

        worksheet = Panel(
            title="Entitlement worksheet: " + result.subject,
            rows=(
                Row("Jurisdiction / type", result.jurisdiction + " / " + result.employment_type),
                Row("Verdict status", result.status.value),
                Row("Entitled days", _num(result.entitled_days)),
                Row("Taken days", _num(result.taken_days)),
                Row(
                    "Balance days",
                    _num(result.balance_days),
                    "bad" if result.requires_human_review else "ok",
                ),
                Row("Approval path", " -> ".join(result.approval_path)),
            ),
            note=(
                "Every number is computed by the pure engine from a versioned, cited rule pack "
                "and is replayable. A model would narrate this worksheet; it never produces a "
                "day count or a verdict."
            ),
            tone="bad" if result.requires_human_review else "ok",
        )
        decision = Panel(
            title="Decision",
            rows=(
                ...
                Row(
                    "Routed to review",
                    review_ref or "not routed (auto-approved)",
                    "ok" if consistent else "bad",
                ),
                Row("Attributed to", ACTOR),
            ),
            note=(
                "A termination-linked or contested (overdrawn) result is consequential and routed "
                "to a human; a plain within-entitlement balance is auto-approved."
            ),
        ...
        evidence = Panel(
            ...
            note="Every claim carries its source clause. An uncited number is a hallucination.",
        )
        facts = {
        ...
        return [worksheet, decision, evidence], facts
...
def _exit_review(container: Any) -> Any:
    ...
    return container.review_router.route(
        models.TriageResult(
            subject="EMP-7788 (FICTIONAL)",
            severity=kernel.Severity.HIGH,
            decision=kernel.Decision.ESCALATED,
            summary="EMP-7788 (FICTIONAL): termination-linked entitlement",
