...
| `rename_fork.py` | Not a demo script: the one-pass fork rebrand, run ONCE by an adopter. It rewrites the package name, the console-script name, the `POLICYHR` env-var stem, the Terraform resource-id stem and the distribution id across the tree, prints its plan and writes nothing without `--yes`. Like everything else here it is offline and stdlib-only. See [`../docs/ADOPTING.md`](../docs/ADOPTING.md) for the human decisions it deliberately does not make. |
