...
Rewrites the python package name, the console-script name, the ``POLICYHR`` env-var prefix,
...
Markdown prose alone (pass ``--include-docs`` to sweep it too).
    ...
    # Apply it:
    python scripts/rename_fork.py --package acme_hr_copilot --cli acme-hr \
        --env-prefix ACME --resource acme-hr --yes
...
# --------------------------------------------------------------------------------------- #
_OLD_PACKAGE = "hr_policy_answers"
_OLD_CLI = "hr_policy_answers"
_OLD_ENV_STEM = "POLICYHR"
_OLD_RESOURCE = "h2-svc"
_OLD_DIST = "hr-policy-answers"
...
def _rules(args: argparse.Namespace) -> list[tuple[str, str, str]]:
    ...
    return [
        ...
        # The env-var prefix. Matched WITHOUT the trailing underscore so both CONTROLROOM_PROFILE
...
def main() -> int:
    ...
    parser.add_argument("--cli", required=True, help="new console-script name, e.g. acme-hr")
    parser.add_argument("--env-prefix", required=True, help="new env-var prefix stem, e.g. ACME")
    parser.add_argument(
        "--resource", required=True, help="new Terraform name_prefix stem, e.g. acme-hr"
    )
    parser.add_argument(
