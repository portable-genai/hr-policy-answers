...
from hr_policy_answers.adapters.gcp.identity import (
    _AUDIENCE_ENV,
...
from hr_policy_answers.ports.identity import (
    VERIFIED,
...
_PROFILE_ENV = "POLICYHR_PROFILE"
_SETTINGS_ENV = "POLICYHR_SETTINGS"

#: The API module, named once so no line's length depends on the rendered package name.
_API_MODULE = "hr_policy_answers.api.app"
...
def _managed_app(monkeypatch: pytest.MonkeyPatch, tmp_path: Any, audience: str | None) -> Any:
    ...
    caller. Setting the variable to an EMPTY value is a different state and refuses at import
...
def test_an_emptied_audience_is_refused_by_the_loader_and_never_serves(
    ...
    ``iap_audience: ${POLICYHR_IAP_AUDIENCE:-}`` goes through the settings loader, which
    delegates to ``envread.setting_or_default``. UNSET still yields the empty string, which the
    adapter turns into a 503 naming the variable (the test above). SET-AND-EMPTY is a third
    state and the loader refuses it outright, so the mistake surfaces in the deploy that made it
    seconds earlier rather than as a value nobody reviewed.

    The API module still IMPORTS, because ``end_user_auth_kind`` swallows any lookup failure and
    resolves to ``CLIENT_ASSERTED`` on purpose: a guard that switches off because a lookup raised
