...
def _persisted_rows() -> list[dict[str, Any]]:
    """Run the REAL pipeline over the case that carries a planted id; return the WORM rows."""
    container = build_container(Settings(profile="local", audit_path=":memory:"))
    TriageService(container.audit, tracer=container.tracer).triage(
        sample_cases.PII_CASE, actor=sample_cases.ACTOR
    )
...
def _unredacted(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in mutated:
        ...
        if citations:
            citations[0]["snippet"] = f"urgent breach, NRIC {sample_cases.PLANTED_NRIC} on file"
    return mutated
