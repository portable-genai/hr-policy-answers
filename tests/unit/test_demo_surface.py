...
def test_the_demo_data_is_obviously_fictional() -> None:
    ...
    for case in cases:
        ref = case.facts.employee_ref
        assert "FICTIONAL" in ref, ref + " does not announce itself as fake"
    blob = " ".join(case.facts.employee_ref for case in cases) + demo.ACTOR + demo.TENANT
    for real_looking in ("@gmail.", "@outlook.", "@yahoo."):
        assert real_looking not in blob
    # The verified principal is an .example address; no case carries a routable identifier.
    assert ".example" in blob, "the demo actor should use an .example domain"
