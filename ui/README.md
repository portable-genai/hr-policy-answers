...
A Next.js App Router console for Policy and HR Copilot (H2).
...
A value that is present but names nothing usable refuses too: `UI_FRAME_ANCESTORS=","` is
refused rather than quietly downgraded to the shipped default.

A value that names EVERYBODY refuses on the same path, and refusing is the point. Any entry
containing an asterisk is rejected, not dropped from the list: `*`, `'*'`, `*.*`, and the partial
forms `https://*.example`, `*.example` and `https://*` alike, whether the entry stands alone or
sits beside a legitimate origin. A partial wildcard is no safer than a bare one, because it trusts
every subdomain including one an attacker took over or one that serves user content, and no real
origin contains the character, so nothing correct is turned away. The literal `null` is refused as
its own named token: it is a wildcard by behaviour rather than by spelling, since a sandboxed
iframe presents a null origin.
